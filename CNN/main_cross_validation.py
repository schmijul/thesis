...
number_of_cross_validation_rounds = 10
shuffleSeed = 1
...
#########################################
# set random seed here such that 'methods' are comparable
np.random.seed(shuffleSeed)
all_scaled_errors = []
all_rescaled_errors = []
intermediate_maes_scaled = np.empty((number_of_cross_validation_rounds, ))
intermediate_maes_rescaled = np.empty((number_of_cross_validation_rounds, ))
maes_scaled_per_iteration = np.empty((number_of_cross_validation_rounds, ))
meas_rescaled_per_iteration = np.empty((number_of_cross_validation_rounds, ))

for cross_val_index in range(number_of_cross_validation_rounds):
    ...
    main_map_random_NN = griddata(points, values, (grid_x, grid_y), method=pre_processing_method).T
    ...
    my_callbacks = [keras.callbacks.ModelCheckpoint(
        filepath=trainedModelPath + '/best_model_cv_iteration' + str(cross_val_index) + '.h5',
        monitor='val_loss',
        mode='min',
        save_best_only=True),
    ...
    best_model = keras.models.load_model('TrainedModels/best_model_cv_iteration' + str(cross_val_index) + '.h5')
    ...
    maes_scaled_per_iteration[cross_val_index] = np.mean(abs(scaled_errors))
    meas_rescaled_per_iteration[cross_val_index] = np.mean(abs(rescaled_errors))

    all_scaled_errors = np.append(all_scaled_errors, np.reshape(scaled_error_array, (portion_size**2, )))
    all_rescaled_errors = np.append(all_rescaled_errors, np.reshape(rescaled_error_array, (portion_size ** 2, )))

    intermediate_test_mae_scaled = np.mean(abs(all_scaled_errors))
    intermediate_test_mae_rescaled = np.mean(abs(all_rescaled_errors))

    intermediate_maes_scaled[cross_val_index] = intermediate_test_mae_scaled
    intermediate_maes_rescaled[cross_val_index] = intermediate_test_mae_rescaled

    print('Intermediate Test MAE (scaled):', intermediate_test_mae_scaled)
    print('Intermediate Test MAE:', intermediate_test_mae_rescaled)

# MAE of scaled error
test_mae_scaled = np.mean(abs(all_scaled_errors))
test_mae_rescaled = np.mean(abs(all_rescaled_errors))

print('Final Test MAE (scaled):', test_mae_scaled)
print('Final Test MAE:', test_mae_rescaled)

plt.plot(meas_rescaled_per_iteration)
plt.plot(intermediate_maes_rescaled)
plt.title('Test MAE')
plt.ylabel('MAE')
plt.xlabel('Cross-validation iteration')
plt.legend(('Single Test MAEs', 'Combined Test MAEs'))
plt.show()
