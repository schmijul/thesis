...
def gridinterpolation(knownpoints, unknownpoints, method='linear', verbose=False):
    ...
    stepsize_x = 1#x_values.unique()[1] - x_values.unique()[0]
    stepsize_y = 1#y_values.unique()[1] - y_values.unique()[0]
    ...
    if verbose:
        print(f'interpolated grid with shape {lin_interpol.shape}')
        print(f'lin interpol stacked shape : {lin_interpol.stack(dropna=False).reset_index().shape}')
